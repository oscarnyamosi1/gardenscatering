...
export default function ContactPage() {
  ...
  const onSubmit = async (data: z.infer<typeof contactSchema>) => {
    ...
    try {
      const whatsappNumber = companyData.socialLinks.whatsapp.replace(/\D/g, '');
      const fullMessage = `*New Contact Form Message*\n\n🏷 Name: ${data.name}\n📧 Email: ${data.email}\n📞 Phone: ${data.phone}\n📌 Subject: ${data.subject}\n\n💬 Message:\n${data.message}`.trim();
              ...
              <div className="glass-card p-8 rounded-3xl">
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4 text-muted-foreground">
                    ...
                    <div>
                      <p className="font-medium text-foreground">{companyData.name} HQ</p>
                      <p>{companyData.location.street}, {companyData.location.city}</p>
                      <p>{companyData.location.city}, {companyData.location.country}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-muted-foreground">
                    <Phone className="w-5 h-5 text-primary shrink-0" />
                    <p>{companyData.businessContacts.phone}</p>
                  </div>
                  <div className="flex items-center gap-4 text-muted-foreground">
                    <Mail className="w-5 h-5 text-primary shrink-0" />
                    <p>{companyData.businessContacts.email}</p>
                  </div>
                  <div className="flex items-start gap-4 text-muted-foreground">
                ...
                <div className="w-full h-48 bg-secondary/50 rounded-3xl border border-border flex items-center justify-center mt-6">
