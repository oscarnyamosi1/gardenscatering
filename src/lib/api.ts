...
async function mockApiHandler(method: string, endpoint: string, data: any) {
  ...
  if (endpoint === '/auth/login' && method === 'POST') {
    ...
    sessionStorage.setItem('gardens-session', JSON.stringify(sessionData));
  ...
  if (endpoint === '/auth/me' && method === 'GET') {
    const session = sessionStorage.getItem('gardens-session');
  ...
  if (endpoint === '/auth/logout' && method === 'POST') {
    sessionStorage.removeItem('gardens-session');
    return { success: true };
  }
